...
function ThreeDCard({
  ...
  const cardStyle = {
    ...
    boxShadow: enableShadow
      ? `${transform.shadowX}px ${transform.shadowY}px ${shadowBlur}px rgba(0, 0, 0, 0.25)`
      : 'none',
  ...
  const glowStyle = enableGlow
    ? {
        background: `radial-gradient(circle at ${transform.glowX}% ${transform.glowY}%, rgba(161, 128, 128, ${glowOpacity}), transparent)`,
        opacity: transform.isHovered ? 1 : 0,
        transition: 'opacity 0.5s ease-in-out'
      }
    : {};
  ...
  return (
    <div style={{ perspective: '1000px' }} className={className}>
      ...
      >
        ...
        <div
          className="absolute inset-0 border-2 border-[#a18080]/30 rounded-2xl pointer-events-none"
          aria-hidden="true"
        />
